# StatusWindow_Excel.ps1 - Ventana de estado EXCEL
...
try {
    ...
    # Posicionar en esquina inferior izquierda (debajo de YANK)
    $screenHeight = [System.Windows.SystemParameters]::PrimaryScreenHeight
    $window.Left = 20
    $window.Top = $screenHeight - 220  # 120px más arriba que NVIM

    # Inicialmente oculta
    $window.Visibility = "Hidden"

    # Archivo de estado
    $statusFile = Join-Path (Split-Path $PSScriptRoot -Parent) "status_excel_commands.json"
    ...
    function Update-Status {
        ...
        if ($show) {
            ...
            $window.UpdateLayout()
            $window.Top = $screenHeight - $window.ActualHeight - 140  # 140px desde abajo
        } else {
            $window.Visibility = "Hidden"
        }
    }
