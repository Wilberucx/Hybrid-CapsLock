# StatusWindow_Visual.ps1 - Ventana de estado VISUAL
...
try {
    ...
    # Posicionar en esquina inferior izquierda (debajo de NVIM)
    $screenHeight = [System.Windows.SystemParameters]::PrimaryScreenHeight
    $window.Left = 20
    $window.Top = $screenHeight - 140  # 40px más arriba que NVIM

    # Inicialmente oculta
    $window.Visibility = "Hidden"

    # Archivo de estado
    $statusFile = Join-Path (Split-Path $PSScriptRoot -Parent) "status_visual_commands.json"
    ...
    function Update-Status {
        ...
        if ($show) {
            ...
            $window.UpdateLayout()
            $window.Top = $screenHeight - $window.ActualHeight - 60  # 60px desde abajo
        } else {
            $window.Visibility = "Hidden"
        }
    }
